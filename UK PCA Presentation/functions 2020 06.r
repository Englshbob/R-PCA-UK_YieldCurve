...
##	axis(side=4, at = pretty(range(prcomp.obj$x[,2])))###
...
plot(prcomp.obj$x[,2]~datelabels, type = "l", col = colours.for.barcharts[2],axes=FALSE, frame.plot=TRUE, main = "Magnification of Factor 2 to n")

	for (i in 3:number.of.factors.used)
	{
	  points( prcomp.obj$x[,i]~datelabels, type = "l",col = colours.for.barcharts[i],axes=FALSE, frame.plot=TRUE,) # adds to the previous chart
	  #time <- 1:dim(rates)[1] # creates a time stamp. In the future need to 
	  #change this code to create dates and use a chron datatype
	}
	legend("topleft", legend=paste("Eig",2:number.of.factors.used),fill = colours.for.barcharts[2:number.of.factors.used])
	grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted", lwd = par("lwd"), equilogs = TRUE)
	#rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],col = "light gray")
...
#	dev.new()
	...
	plot(prcomp.obj$x[,factor], type = "l", col = colours.for.barcharts[factor],axes=FALSE, frame.plot=TRUE, main = Main.Chart.Label)
	grid(nx = NULL, ny = NULL, col = "lightgray", lty = "dotted", lwd = par("lwd"), equilogs = TRUE)
	#rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],col = "light gray")
	axis(side=1, at = datelabels, labels = format(datelabels,"%m-%y"), tick = TRUE)#  line = NA, #side puts the axis on the bottom of the chart
...
{
number.of.factors.used<-dim(prcomp.obj$rotation)[2]
variances.of.principal.coponents<-prcomp.obj$sdev^2
percent.of.risk.explained<-variances.of.principal.coponents/sum(variances.of.principal.coponents)
bars <- percent.of.risk.explained[1:number.of.factors.used]
...
barplot(bars,ylim=c(0,1), col=colours.for.barcharts,axis.lty=1,ylab=" % of Variance Explained", main="Scaled Eigenvalues",names=paste("Eig",1:number.of.factors.used),axes=TRUE) # need to manually calculate
grid(nx = NULL, ny = 20, col = "lightgray", lty = "dotted", lwd = par("lwd"), equilogs = TRUE)
#rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],col = "light gray")
}
...
x<-tapply(X=time.series,INDEX=interval.in.which.observation.sits,FUN=median)[2:(numItv-1)] # finds the middle of each range
y<-tapply(X=difference.ts,INDEX=interval.in.which.observation.sits,FUN=median)[2:(numItv-1)] # finds the middle of each range
...
#y<-c("red","blue","green","black","purple")[1:dim(prcomp.obj$rotation)[2]]#original working code for four colours only
#colours<-rep(y,dim(prcomp.obj$rotation)[1])#original working code for four colours only
	num.of.variables <- dim(prcomp.obj$rotation)[2]
	...
	ticker.required<-strip.ticker.suffixes(ticker.required)
	row.names(factor.loadings)<-c(ticker.required)
  yaxislimits<-2*c(round(min(factor.loadings),digits=0),round(max(factor.loadings),digits=0))
...
if (type =="b")
{
#	barplot(t(factor.loadings),ylab="Exposure of Variable to Factor", xpd=FALSE,col=colours, main="Factor Exposures",beside=TRUE,ylim=c(round(min(factor.loadings),digits=1),round(max(factor.loadings),digits=1)))
#  barplot(t(factor.loadings),ylab="Exposure of Variable to Factor", col=colours, main="Factor Exposures",beside=TRUE,ylim=(c(-1,1)))
   barplot(t(factor.loadings),ylab="Exposure of Variable to Factor", col=colours, main="Factor Exposures",beside=TRUE,ylim=yaxislimits)
  
  }
else
{
	plot(t(factor.loadings)[1,],lwd=3,ylab="Exposure of Variable to Factor",type="l", col=colours[1], main="Factor Exposures",ylim=c(round(min(factor.loadings),digits=1),round(max(factor.loadings),digits=2)))
...
#########################################################################################################################################################################
## improved chart for forecasting intervals (Uses library(fanplot))
#########################################################################################################################################################################


plot.forecast.OU2<-function(historic.values,forecasted.values,std.dev)
{
  ...
  #bands<-cbind(as.ts(upper2sd.plot.vals[fcast.range]),as.ts(upper1sd.plot.vals[fcast.range]),as.ts(lower1sd.plot.vals[fcast.range]),as.ts(lower2sd.plot.vals[fcast.range]))
  #bands<-as.ts(cbind(upper2sd.plot.vals[fcast.range],upper1sd.plot.vals[fcast.range],lower1sd.plot.vals[fcast.range],lower2sd.plot.vals[fcast.range]))
  #hist.plot.vals<-runif(120)-0.5
  u2<-upper2sd.values
  u1<-upper1sd.values
  l1<-lower1sd.values
  l2<-lower2sd.values
  ...
  upperlim<-max(rbind(historic.values,u2))
  lowerlim<-min(rbind(historic.values,l2))
  ylimits<-c(upperlim,lowerlim)
  bands<-cbind(u2,u1,l1,l2)
  
  plot(hist.plot.vals, type = "l", col = "black", main = "Forecast Graph",ylim=ylimits)
  percentile.list<-c(95,66,33,5)
  library(fanplot)
  fan(data=t(bands),start=num.hist.obs, anchor=hist.plot.vals[num.hist.obs], fan.col = colorRampPalette(c("green", "gray90")), ln = NULL, rlab = NULL)
    #, data.type="values",probs=percentile.list
...
#########################################################################################################################################################################
## this plots the series but fits the OU on a shorter dataset so you can see how well the model works
#########################################################################################################################################################################
 
plot.forecast.OU.test<-function(complete.timeseries,nbr.of.heldback.observations)
{
  timeseries<-complete.timeseries[1:(length(complete.timeseries)-nbr.of.heldback.observations)]
  ...
  #plot.forecast.OU2(timeseries,fcast$expected.values,fcast$standard.deviations)
  
  historic.values<-timeseries
  forecasted.values<-fcast$expected.values
  std.dev<-fcast$standard.deviations
  
  
  num.plotted.hist.obs  <- length(as.array(historic.values))+nbr.of.heldback.observations
  plotted.hist.range <- 1:num.plotted.hist.obs
  ...
  hist.plot.vals<-array(NA,dim=num.hist.obs+num.fcast.obs)
  hist.plot.vals[plotted.hist.range]<-complete.timeseries
...
###note i switched this back from ou2 to ou to get it to work
}
